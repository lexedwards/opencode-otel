...
export type Options = SignalOptions & {
  traces?: SignalOptions
  metrics?: SignalOptions
  providerNames?: Record<string, string>
  executionExpiryMillis?: number
...
export function exporterSecrets(signal: SignalConfig): PrivateOptions {
  return secrets.get(signal) ?? { headers: {} }
}
...
function resolveSignal(signal: Signal, options: Record<string, unknown>, env: Environment): SignalConfig | undefined {
  ...
  const mergedHeaders = {
    ...headers(env[generic + "HEADERS"], env, false),
    ...headers(env[prefix + "HEADERS"], env, false),
    ...headers(selectedHeaders, env, true)
  }
  ...
  const resolvedCertificate =
    certificate === undefined
      ? undefined
      : scoped.certificate !== undefined || options.certificate !== undefined
        ? secret(certificate, "certificate", env)
        : text(certificate, "certificate")
  const resolvedClientCertificate =
    clientCertificate === undefined
      ? undefined
      : scoped.clientCertificate !== undefined || options.clientCertificate !== undefined
        ? secret(clientCertificate, "client certificate", env)
        : text(clientCertificate, "client certificate")
  const resolvedClientKey =
    clientKey === undefined
      ? undefined
      : scoped.clientKey !== undefined || options.clientKey !== undefined
        ? secret(clientKey, "client key", env)
        : text(clientKey, "client key")
  ...
  if (
    [privateOptions.certificate, privateOptions.clientCertificate].some(
      (value) => value !== undefined && !/-----BEGIN CERTIFICATE-----[\s\S]+-----END CERTIFICATE-----/.test(value)
    )
  )
    throw Error("certificate")
  if (
    privateOptions.clientKey !== undefined &&
    !/-----BEGIN (?:RSA |EC |ENCRYPTED )?PRIVATE KEY-----[\s\S]+-----END (?:RSA |EC |ENCRYPTED )?PRIVATE KEY-----/.test(privateOptions.clientKey)
  )
    throw Error("client key")
  ...
  const batchQueueSize =
    signal === "traces" ? positive(scoped.batchQueueSize ?? options.batchQueueSize ?? env.OTEL_BSP_MAX_QUEUE_SIZE ?? 2048, "batch queue") : undefined
  const batchMaxSize =
    signal === "traces" ? positive(scoped.batchMaxSize ?? options.batchMaxSize ?? env.OTEL_BSP_MAX_EXPORT_BATCH_SIZE ?? 512, "batch size") : undefined
  if (batchQueueSize !== undefined && batchMaxSize !== undefined && batchMaxSize > batchQueueSize) throw Error("batch size")
  const batchDelayMillis =
    signal === "traces" ? positive(scoped.batchDelayMillis ?? options.batchDelayMillis ?? env.OTEL_BSP_SCHEDULE_DELAY ?? 5000, "batch delay") : undefined
  const batchTimeoutMillis =
    signal === "traces" ? positive(scoped.batchTimeoutMillis ?? options.batchTimeoutMillis ?? env.OTEL_BSP_EXPORT_TIMEOUT ?? 30000, "batch timeout") : undefined
  const exportIntervalMillis =
    signal === "metrics"
      ? positive(scoped.exportIntervalMillis ?? options.exportIntervalMillis ?? env.OTEL_METRIC_EXPORT_INTERVAL ?? 60000, "metric interval")
      : undefined
  const metricTimeoutMillis =
    signal === "metrics"
      ? positive(scoped.metricTimeoutMillis ?? options.metricTimeoutMillis ?? env.OTEL_METRIC_EXPORT_TIMEOUT ?? 30000, "metric timeout")
      : undefined
...
export function resolveConfig(options: unknown, env: Environment = process.env): Config {
  ...
  const config: Config = {
    diagnostics: [],
    executionExpiryMillis: 24 * 60 * 60_000,
    propagateTraceContext: false,
    capture: {
      inputMessages: umbrella,
      outputMessages: umbrella,
      systemInstructions: umbrella,
      toolDefinitions: umbrella,
      toolArguments: false,
      toolResults: false,
      errorMessages: false,
      stackTraces: false,
  ...
  try {
    ...
    for (const key of ["redactKeys", "redactPatterns"] as const) {
      if (capture[key] === undefined) continue
      if (!Array.isArray(capture[key]) || !capture[key].every((item) => typeof item === "string" && item.length <= 256) || capture[key].length > 64)
        throw Error("capture")
      config.capture[key] = [...capture[key]]
    }
  ...
  try {
    ...
    config.diagnostics.push("Trace propagation invalid; disabled")
  }
  try {
    ...
    config.diagnostics.push("Execution expiry invalid; using 24-hour default")
  }
  try {
    const names = record(record(options).providerNames)
    config.providerNames = Object.fromEntries(
    ...
    config.diagnostics.push("Provider name overrides invalid; defaults used")
  }
...
export function establishConfig(registry: Registry, proposed: Config): { config: Config; diagnostic?: string; release: () => void } {
  const fingerprint = (config: Config) =>
    JSON.stringify([
      config.traces,
      config.metrics,
      config.providerNames,
      config.executionExpiryMillis,
      config.propagateTraceContext,
      config.capture,
      config.traces && exporterSecrets(config.traces),
      config.metrics && exporterSecrets(config.metrics)
    ])
  const diagnostic =
    registry.current && fingerprint(registry.current) !== fingerprint(proposed)
