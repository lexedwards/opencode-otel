...
import { type CompactionEvent, createTelemetry, type ExecutionEvent, type ModelEvent, type PermissionEvent, type SessionRelationEvent } from "./telemetry"
      ...
      try {
        hooks.push(
          await ctx.tool.hook("execute.before", (event) => {
            try {
              pipeline?.execution.toolBefore(event)
            } catch {
              /* fail open */
            }
          })
        ...
        hooks.push(
          await ctx.tool.hook("execute.after", (event) => {
            try {
              pipeline?.execution.toolAfter(event)
            } catch {
              /* fail open */
            }
          })
        ...
        try {
          registrations.push(
            await ctx.session.hook("context", (event) => {
              try {
                pipeline?.execution.captureContext(event, "primary")
              } catch {
                /* fail open */
              }
            })
          ...
          registrations.push(
            await ctx.session.hook("compaction", (event) => {
              try {
                pipeline?.execution.captureContext(event, "compaction")
              } catch {
                /* fail open */
              }
            })
      ...
      void (async () => {
        try {
          for await (const event of ctx.event.subscribe({ signal: controller.signal })) {
            ...
            if (
              !(
                eventType.startsWith("session.execution.") ||
                eventType.startsWith("session.step.") ||
                ["session.compaction.started", "session.compaction.ended", "session.compaction.failed"].includes(eventType) ||
                eventType === "session.retry.scheduled" ||
                eventType === "session.created" ||
                eventType === "session.forked" ||
                eventType === "session.text.ended" ||
            ...
            try {
              ...
              else if (eventType === "session.text.ended")
                pipeline?.execution.onTextEnded(
                  event as { type: "session.text.ended"; data: { sessionID: string; assistantMessageID: string; ordinal: number; text: string } }
                )
