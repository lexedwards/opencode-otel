import { ROOT_CONTEXT, isSpanContextValid, trace, type SpanContext } from "@opentelemetry/api"
...
export function injectTraceHeaders(headers: Headers | Record<string, string>, context: SpanContext): boolean {
  ...
  w3c.inject(trace.setSpanContext(ROOT_CONTEXT, context), values, { set: (carrier, key, value) => { carrier[key] = value } })
  if (!values.traceparent) return false
  if (headers instanceof Headers) {
    if (values.tracestate) headers.set("tracestate", values.tracestate)
    try { headers.set("traceparent", values.traceparent) }
    catch (error) { if (values.tracestate) headers.delete("tracestate"); throw error }
  } else {
    if (values.tracestate) headers.tracestate = values.tracestate
    try { headers.traceparent = values.traceparent }
    catch (error) { if (values.tracestate) delete headers.tracestate; throw error }
  }
  return true
}
