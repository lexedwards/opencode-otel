...
test("capture categories default off, umbrella enables them, and granular opt-outs win", () => {
  ...
  expect(
    resolveConfig({ capture: { outputMessages: false, toolDefinitions: false } }, { OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT: "true" }).capture
  ).toMatchObject({ inputMessages: true, outputMessages: false, systemInstructions: true, toolDefinitions: false })
...
test("redaction precedes UTF-8 bounding, retains valid recent messages, and excludes binary", () => {
  const pipeline = new PrivacyPipeline({
    ...
    errorMessages: false,
    stackTraces: false,
    redactKeys: ["sessionSecret"],
    redactPatterns: ["bearer-[0-9]+"]
  })
  const value = pipeline.value({ nested: { password: "hidden", sessionSecret: "hidden-again", data: "bearer-1234", safe: "🦊".repeat(3000) } }) as Record<
    string,
    any
  >
  ...
  const messages = pipeline.messages([
    {
      ...
      content: [
        ...
        {
          type: "media",
          media: { source: { type: "url", url: "https://user:password@example.test/image.png?apiKey=secret#private", mediaType: "image/png" } }
        },
  ...
  coordinator.captureContext(
    {
      ...
      tools: { secret: { description: "private definition" } }
    },
    "primary"
  )
  coordinator.onModelEvent({
...
test("primary and compaction content attach only to their model spans when selected", async () => {
  ...
  const config = resolveConfig(
    {
      ...
      capture: { inputMessages: true, outputMessages: true, systemInstructions: true, toolDefinitions: true, redactKeys: ["apiKey"] }
    },
    {}
  )
  ...
  coordinator.captureContext(
    {
      sessionID: "session",
      model: { id: "model", providerID: "openai" },
      system: [{ type: "text", text: "system-safe" }],
      messages: [
        {
          role: "user",
          content: [
            { type: "text", text: "hello" },
            { type: "tool-call", name: "search", input: { apiKey: "secret-key" } }
          ]
        }
      ],
      tools: { search: { description: "look up", input: { properties: { apiKey: { default: "secret-key" } } } } }
    },
    "primary"
  )
  coordinator.onModelEvent({
  ...
  coordinator.captureContext(
    {
      ...
      model: { id: "model", providerID: "openai" },
      system: [],
      messages: [{ role: "user", content: [{ type: "text", text: "incomplete transcript" }] }]
    },
    "compaction"
  )
  coordinator.onCompactionEvent({
    type: "session.compaction.ended",
    id: "compact-end",
    created: 1900,
    data: { sessionID: "session", reason: "auto", text: "summary-safe", recent: "private recent" }
  })
  coordinator.onEvent({ type: "session.execution.succeeded", id: "root-end", created: 2000, data: { sessionID: "session" } }, "project")
  await telemetry.traces?.forceFlush()
  ...
  expect(JSON.parse(model.attributes["gen_ai.input.messages"] as string)).toEqual([
    {
      role: "user",
      parts: [
        { type: "text", content: "hello" },
        { type: "tool_call", name: "search", arguments: { apiKey: "[redacted]" } }
      ]
    }
  ])
  expect(JSON.parse(model.attributes["gen_ai.output.messages"] as string)).toEqual([
    { role: "assistant", parts: [{ type: "text", content: "answer" }], finish_reason: "unknown" }
  ])
  ...
  expect(JSON.parse(compact.attributes["gen_ai.output.messages"] as string)).toEqual([
    { role: "assistant", parts: [{ type: "text", content: "summary-safe" }], finish_reason: "stop" }
  ])
  ...
  c.captureContext(
    {
      ...
      model: { id: "model", providerID: "openai" },
      system: [],
      messages: Array.from({ length: 20 }, (_, index) => ({ role: "user", content: [{ type: "text", text: `${index}:` + "z".repeat(3500) }] }))
    },
    "primary"
  )
  c.onModelEvent({
