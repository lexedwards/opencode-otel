...
test("compaction is a private client sibling during execution and a standalone trace when manual", async () => {
  ...
  const send = (type: "session.compaction.started" | "session.compaction.ended", sessionID: string, id: string, created: number) => pipeline.execution.onCompactionEvent({ type, id, created, data: { sessionID, reason: sessionID === "session-1" ? "auto" : "manual", recent: "secret transcript", text: "secret summary", model: { id: "claude-test", providerID: "anthropic" }, tokens: { input: 2, output: 3, reasoning: 0, cache: { read: 1, write: 1 } }, cost: 0.01 } })
  send("session.compaction.started", "session-1", "compact-start", 1100)
  send("session.compaction.started", "session-1", "compact-start", 1100)
  pipeline.execution.onCompactionEvent({ type: "session.compaction.delta", id: "compact-delta", created: 1200, data: { sessionID: "session-1", reason: "auto", text: "secret partial summary" } } as any)
...
test("configured expiry abandons stale executions and standalone compactions once", async () => {
  ...
  pipeline.execution.onModelEvent({ type: "session.step.started", id: "stale-model", created: 1100, data: { sessionID: "session-1", assistantMessageID: "msg-stale", model: { id: "test", providerID: "openai" } } })
  pipeline.execution.onCompactionEvent({ type: "session.compaction.started", id: "standalone-start", created: 1200, data: { sessionID: "manual", reason: "manual", recent: "secret" } })
  now = 5900
...
test("terminal events arriving before starts are correlated once when timestamps agree", async () => {
  ...
  pipeline.execution.onModelEvent({ type: "session.step.ended", id: "reordered-step-end", created: 5200, data: { sessionID: "primary", assistantMessageID: "msg-reordered", tokens: { input: 2, output: 1, reasoning: 0, cache: { read: 0, write: 0 } } } })
  pipeline.execution.onModelEvent({ type: "session.step.started", id: "reordered-step-start", created: 4200, data: { sessionID: "primary", assistantMessageID: "msg-reordered", model: { id: "model", providerID: "openai" } } })
  pipeline.execution.onCompactionEvent({ type: "session.compaction.ended", id: "reordered-compact-end", created: 5700, data: { sessionID: "primary", reason: "auto", model: { id: "model", providerID: "openai" } } })
  pipeline.execution.onCompactionEvent({ type: "session.compaction.started", id: "reordered-compact-start", created: 5400, data: { sessionID: "primary", reason: "auto", recent: "secret" } })
